/**
 * \brief Fonction de communication avec le MPU6050
 *
 *
 *
 */
...
/**
 * \brief Pointeur sur le singleton
 *
 * Explication : pour pouvoir retrouver le pointeur sur l'objet dans l'interruption, il est nécessaire
 * de créer un pointeur sur l'objet.
 * Ce pointeur doit être unique (singleton), il n'y aura donc pas d'autre MPU6050 accessible
 */
...
/**
 * \brief Read multi bytes over I2C
 *
 *  * \todo : a supprimer
 */
 ...
 * \brief Constructeur
 *
 * Initialisation des variables seulement,
 * pas de création ni initialisation de la liaison I2C.
 * La liaison est crée par CubeMx
 *
 */
MPU6050::MPU6050 (I2C_HandleTypeDef *i2c_handle, MPU6050_Device_t deviceNumber, MPU6050_Accelerometer_Range_t accel,
...
/*!
 * \brief Initialisation de la communication
 *
 * \todo rajouter les configs et les filtres supprimés lors des essais cartes avec PB de capa
 */
MEMS_Result_t MPU6050::initialisation (void)
{
  ...
  if (HAL_I2C_IsDeviceReady (hi2c, deviceAddress, 2, I2C_TIMEOUT_FLAG) != HAL_OK)
    {
      // if not : restart I2C with 16 manual clocks
      ...
      // MX_I2C1_Init(); // reinitialisation de l'I2C par la fonction générée par CubeMX impossible car statique
      //hi2c->Instance = I2C1;
...
//      if (HAL_I2C_IsDeviceReady (hi2c, deviceAddress, 2, I2C_TIMEOUT_FLAG) != HAL_OK)
      if (HAL_I2C_IsDeviceReady (hi2c, deviceAddress, 2, 1000) != HAL_OK)
...
/**
 * \brief Request data over I2C
 *
 * Initialisation de communication I2C pour la lecture par DMA des registres internes du MPU6050.
 * L'adresse du premier registre est demandée en lecture, puis les adresses suivantes sont automatiquement incrémentées
 * Les registres lus sont ceux contenant les résultats des mesures du gyro, de l'accelero et de la température
 * La fin de la réception va générer une interruption
 *
 */
 ...
 *
 * Cette fonction est appelée par l'interruption de fin de réception en Callback
 * Si les données étaient correctement rangées, elle pourrait être inutile
 */
...
/**
 * \brief Selftest ou autotest
 *
 * Fonction récupérée sur //https://github.com/kriswiner/MPU6050/blob/master/STM32F401/MPU6050.h
 *
 * \todo : à tester,
 * \todo : à utiliser sur requete ?
 */
...
// Output scaled accelerometer biases for manual subtraction in the main program
  ...
  accelOffset_X = (int16_t) accel_bias[0];
  accelOffset_Y = (int16_t) accel_bias[1];
  accelOffset_Z = (int16_t) accel_bias[2];

  gyroOffset_X = (int16_t) gyro_bias[0];
  gyroOffset_Y = (int16_t) gyro_bias[1];
  gyroOffset_Z = (int16_t) gyro_bias[2];

}

/**
 * \brief Fonction de calcul des quaternions
 *
 * https://github.com/kriswiner/MPU6050/blob/master/STM32F401/MPU6050.h
